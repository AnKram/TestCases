yii.gii = (function ($) {
    ...
    var initPreviewDiffLinks = function () {
        $('.preview-code, .diff-code, .modal-refresh, .modal-previous, .modal-next').on('click', function () {
            ...
            ajaxRequest = $.ajax({
                ...
                cache: false,
                url: $link.prop('href'),
                data: $('.views-view form').serializeArray(),
    ...
    var checkAllToggle = function () {
        $('#check-all').prop('checked', !$('.views-view-files table .check input:enabled:not(:checked)').length);
    };

    var initConfirmationCheckboxes = function () {
        var $checkAll = $('#check-all');
        $checkAll.click(function () {
            $('.views-view-files table .check input:enabled').prop('checked', this.checked);
        });
        $('.views-view-files table .check input').click(function () {
            checkAllToggle();
        });
        checkAllToggle();
    };
    ...
    var initToggleActions = function () {
        $('#action-toggle').find(':input').change(function () {
            ...
            var $rows = $('.' + this.value, '.views-view-files table').toggleClass('action-hidden', !this.checked);
            if (this.checked) {
    ...
    return {
        init: function () {
            ...
            $('#form-fields').find('input,select,textarea').change(function () {
                $('.views-view-results,.views-view-files').hide();
                $('.views-view button[name="generate"]').hide();
            });
